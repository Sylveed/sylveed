...
{
	public abstract class StorageBase<TKey, TValue>
	{
		...
		public StorageBase(Func<TValue, TKey> primaryKeySelector)
		{
        ...
        protected IStorageIndex<TSubKey, TValue> CreateIndex<TSubKey>(Func<TValue, TSubKey> keySelector)
        {
            return new Index<TSubKey>(this, keySelector);
        }
        
        class Index<TSubKey> : IStorageIndex<TSubKey, TValue>
        {
            readonly Dictionary<TSubKey, HashSet<TKey>> keyMap = new Dictionary<TSubKey, HashSet<TKey>>();

            readonly StorageBase<TKey, TValue> parent;
            readonly Func<TValue, TSubKey> keySelector;

            public Index(StorageBase<TKey, TValue> parent, Func<TValue, TSubKey> keySelector)
            {
                this.parent = parent;
